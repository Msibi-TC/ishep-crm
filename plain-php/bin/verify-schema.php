...
$required = ['users','password_reset_tokens','roles','permissions','role_permissions','user_roles','provinces','professions','membership_types','audit_logs','member_profiles','membership_applications','membership_application_events','document_types','membership_document_requirements','member_documents','member_document_events','membership_fee_schedules','memberships','invoices','invoice_items','payments','payment_allocations','refund_requests','refunds','finance_events','career_opportunities','career_applications','career_application_events','bursary_opportunities','bursary_applications','bursary_application_events'];
...
try {
    ...
    $financeColumns=['membership_fee_schedules'=>['fee_reference','membership_type_id','amount','currency','billing_period','effective_from','effective_to','active'],'memberships'=>['membership_number','user_id','membership_application_id','status','activated_at'],'invoices'=>['invoice_number','membership_id','subtotal','total','amount_paid','balance_due','status'],'payments'=>['payment_reference','amount','currency','manual_reference','status','reversal_of_payment_id','idempotency_key'],'refund_requests'=>['request_reference','payment_id','requested_amount','status'],'refunds'=>['refund_reference','refund_request_id','amount','manual_reference'],'finance_events'=>['entity_type','entity_reference','action']];$missingFinanceColumns=[];foreach($financeColumns as $table=>$expected){$columns=$pdo->prepare('SELECT column_name FROM information_schema.columns WHERE table_schema=? AND table_name=?');$columns->execute([$config['database'],$table]);$diff=array_diff($expected,$columns->fetchAll(PDO::FETCH_COLUMN));if($diff)$missingFinanceColumns[$table]=$diff;}$financeUnique=[];foreach(['membership_fee_schedules'=>'fee_schedules_reference_unique','memberships'=>'memberships_number_unique','invoices'=>'invoices_number_unique','payments'=>'payments_reference_unique','refund_requests'=>'refund_requests_reference_unique','refunds'=>'refunds_reference_unique'] as $table=>$name){$index=$pdo->prepare('SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=? AND table_name=? AND index_name=? AND non_unique=0');$index->execute([$config['database'],$table,$name]);$financeUnique[$table]=(int)$index->fetchColumn()>=1;}$portalColumns=['career_opportunities'=>['opportunity_reference','title','closing_date','status'],'career_applications'=>['application_reference','career_opportunity_id','applicant_user_id','status'],'career_application_events'=>['career_application_id','previous_status','new_status','action'],'bursary_opportunities'=>['opportunity_reference','title','closing_date','status'],'bursary_applications'=>['application_reference','bursary_opportunity_id','applicant_user_id','status'],'bursary_application_events'=>['bursary_application_id','previous_status','new_status','action']];$missingPortalColumns=[];foreach($portalColumns as $table=>$expected){$columns=$pdo->prepare('SELECT column_name FROM information_schema.columns WHERE table_schema=? AND table_name=?');$columns->execute([$config['database'],$table]);$diff=array_diff($expected,$columns->fetchAll(PDO::FETCH_COLUMN));if($diff)$missingPortalColumns[$table]=$diff;}
