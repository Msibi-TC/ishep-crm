...
final class Session
{
    public function start(string $name,int $minutes,string $path,bool $secure=true,string $sameSite='Lax'):void { if(session_status()===PHP_SESSION_ACTIVE)return; $seconds=$minutes*60;$sameSite=in_array($sameSite,['Lax','Strict','None'],true)?$sameSite:'Lax'; ini_set('session.use_strict_mode','1'); ini_set('session.use_only_cookies','1'); ini_set('session.gc_maxlifetime',(string)$seconds); session_name($name); session_save_path($path); session_set_cookie_params(['lifetime'=>$seconds,'path'=>'/','secure'=>$secure,'httponly'=>true,'samesite'=>$sameSite]); session_start(); if(isset($_SESSION['_last_activity'])&&$_SESSION['_last_activity']<time()-$seconds){$_SESSION=[];session_regenerate_id(true);} $_SESSION['_last_activity']=time(); }
